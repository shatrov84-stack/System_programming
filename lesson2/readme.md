# 🎯 **УРОК 2: СТРОКИ И ОБРАБОТКА ОШИБОК В WINAPI**

**Цель урока:**  
Научиться безопасно работать со строками в Windows API и правильно обрабатывать системные ошибки. Освоить конвертацию между кодировками и диагностику ошибок WinAPI.
...
- **Страницы 18-28:** "Working with Strings" — работа со строками в WinAPI
- **Страницы 31-34:** "Handling API Errors" — обработка ошибок
- **Страницы 34-35:** "Defining Custom Error Codes" — пользовательские коды ошибок

### **Ключевые концепции:**

#### **1. Строки в WinAPI:**
- **Unicode vs ANSI:** Windows использует UTF-16 (широкие символы `wchar_t`)
- **Кодовые страницы:** Windows-1251 для русского языка, CP_UTF8 для Unicode
- **Безопасные функции:** `strcpy_s`, `strcat_s`, `sprintf_s` вместо небезопасных аналогов
- **Конвертация:** `MultiByteToWideChar`, `WideCharToMultiByte`

#### **2. Обработка ошибок:**
- **`GetLastError()`:** Получение кода последней ошибки
- **`FormatMessage()`:** Преобразование кода ошибки в читаемое сообщение
- **`SetLastError()`:** Установка собственных кодов ошибок
- **Коды ошибок:** `ERROR_SUCCESS`, `ERROR_FILE_NOT_FOUND`, `ERROR_ACCESS_DENIED`

#### **3. Типы данных Windows:**
- `DWORD` — 32-битное беззнаковое целое
- `LPSTR`, `LPWSTR` — указатели на строки (ANSI и Unicode)
- `BOOL` — логический тип (TRUE=1, FALSE=0)
- `HRESULT` — результат операции (SUCCEEDED/FAILED)
...
1. Почему в WinAPI используется UTF-16 (Unicode), а не однобайтовые кодировки?
2. Чем функция `strcpy_s` безопаснее обычного `strcpy`?
3. Что произойдет, если не проверять результат вызова `GetLastError()` после неудачной операции?
4. Как правильно освобождать память, выделенную функцией `FormatMessage`?
5. Зачем нужна функция `SetLastError()` и в каких случаях ее используют?

### **Часть 2: Практическая работа**

**Задача:** Создать утилиту для работы со строками и диагностики ошибок WinAPI

**Требования к программе:**
1. Настройка русской локали (Windows-1251)
2. Конвертер строк между кодировками
3. Анализатор кодов ошибок WinAPI
4. Проверка существования файлов с обработкой ошибок
5. Демонстрация безопасных строковых операций
...
2. Настройте проект: **C/C++ → Дополнительно → "Компилировать как код C (/TC)"**
3. Вставьте код в `main.c`
4. Нажмите **Ctrl+F5** для запуска
...
## 📋 **АНАЛИЗ ПРОГРАММЫ (задание для ученика):**

1. Почему в функции `CheckFileExists()` используется `GetFileAttributesW()` вместо `GetFileAttributesA()`?
2. Что произойдет, если не вызвать `LocalFree(errorMsg)` после `FormatMessageA()`?
3. Почему в функции `ConvertStrings()` временно меняется кодовая страница вывода?
4. Какая разница между `strcpy_s` и обычным `strcpy`?
5. Зачем нужна функция `SetLastError()` в реальных приложениях?
...
1. Добавьте в функцию `ConvertStrings()` конвертацию в кодировку UTF-8
2. Расширьте `AnalyzeError()` — добавьте поиск ошибки по имени (например, по строке "ACCESS_DENIED")
3. В `CheckFileExists()` добавьте вывод размера файла (используйте `GetFileSizeEx`)

### **Уровень 2: Новые функции**
1. Реализуйте функцию для копирования файла с обработкой всех возможных ошибок
2. Создайте функцию поиска текста в файле с использованием безопасных строковых операций
3. Добавьте функцию для получения информации о системе (версия ОС, имя компьютера)

### **Уровень 3: Продвинутые задачи**
1. Реализуйте простой логгер, который записывает все операции программы в файл
2. Создайте функцию для валидации путей файлов (проверка на недопустимые символы)
3. Добавьте поддержку аргументов командной строки для программы
...
2. ✅ Русский язык отображается корректно
3. ✅ Все 5 функций реализованы и работают
4. ✅ Используются безопасные строковые функции (`_s` версии)
5. ✅ Обработка ошибок WinAPI через `GetLastError()` и `FormatMessage()`

### **Качество кода:**
1. ✅ Структурированный код с комментариями
2. ✅ Нет утечек памяти (особенно после `FormatMessage`)
3. ✅ Проверка возвращаемых значений всех функций
4. ✅ Обработка граничных случаев (пустые строки, неверный ввод)

---

## 📚 **СВЯЗЬ С КНИГОЙ ИОСИФОВИЧА:**

### **Глава 1 (страницы 18-28):**
- Правильная работа со строками в Windows
- Конвертация между кодировками
- Использование безопасных строковых функций

### **Глава 1 (страницы 31-35):**
- Обработка ошибок WinAPI
- Получение и интерпретация кодов ошибок
- Создание пользовательских ошибок

### **Что дальше (Глава 2):**
- Объекты и дескрипторы в Windows
- Ядро объектов и управление ресурсами
...
3. **Примеры работы** программы (скриншоты)
4. **Список изменений** — что добавили/изменили

### **Срок выполнения:** 7-10 дней
### **Обратная связь:** Детальный разбор кода и рекомендации

---

## 💡 **СОВЕТЫ ПО ВЫПОЛНЕНИЮ:**

1. **Начинайте с теории** — прочитайте указанные страницы книги
2. **Тестируйте каждую функцию отдельно** перед объединением
3. **Используйте отладчик** для анализа кодов ошибок
4. **Документируйте сложные моменты** в комментариях
5. **Экспериментируйте** с разными сценариями ошибок

---

**Этот урок заложит фундамент для безопасной работы со строками и корректной обработки ошибок — двух критически важных навыков системного программиста Windows.**
