...
void CreateThreadsWithPriorities()
{
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        ...
        // Создаем поток с использованием _beginthreadex (рекомендуется в CRT)
...
void ParallelArrayProcessing()
{
    ...
    DWORD parallel_time = end_time - start_time;
    ...
    DWORD single_thread_time = end_time - start_time;
    ...
    printf("  - Оптимальное число потоков ≈ числу ядер процессора\n");
...
unsigned __stdcall WorkerThread(void* param)
{
    ...
    // Ожидание события START (если указано)
    ...
    // Сигнализируем о завершении работы (если указано событие)
    if (data->done_event != NULL) {
        SetEvent(data->done_event);
    }
    
    // Освобождаем память и завершаем поток
    free(data);
    return 0;
}
